...
export const postJob = catchAsyncError(async(req,res,next)=>{
    ...
    if(!title || !description || !category || !country || !city || !location){
        return next(new ErrorHandler("Please provide full jon details",400));
    }
    if((!salaryFrom || !salaryTo) && !fixedSalary){
        return next(new ErrorHandler("Cannot enter fixed salary and ranges salary together!"));
    }
    if(salaryFrom && salaryTo && fixedSalary){
...
export const getSinglejob = catchAsyncError(async(req,res,next)=>{
    ...
    try{
        ...
        return next(new ErrorHandler("Invalid ID/Caste Error",400));
    }
})